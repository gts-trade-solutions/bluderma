...
export default function GalleryCaseRow({
  ...
  const consent = consentWithdrawn
    ? { text: "Consent withdrawn", className: "bg-rose-50 text-rose-700" }
    : consentGiven
      ? { text: "Patient agreed", className: "bg-teal-50 text-teal-700" }
      : { text: "Waiting on the patient", className: "bg-amber-50 text-amber-700" };

  return (
    <li className="px-4 py-3.5 sm:px-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="text-sm font-bold text-slate-900">{treatmentName}</p>
        ...
        <div className="flex shrink-0 gap-1.5">
          {status !== GalleryStatus.PUBLISHED && (
            <button
              ...
              className="inline-flex min-h-9 items-center gap-1.5 rounded-full bg-teal-600 px-3 py-1.5 text-xs font-bold text-white transition hover:bg-teal-700 disabled:opacity-40"
            >
          ...
          {status === GalleryStatus.PUBLISHED && (
            <button
              ...
              className="inline-flex min-h-9 items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700 transition hover:bg-slate-200 disabled:opacity-60"
            >
          ...
          <button
            ...
            className="inline-flex min-h-9 items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold text-rose-600 transition hover:bg-rose-50 disabled:opacity-60"
          >
