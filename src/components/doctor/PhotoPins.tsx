...
const field =
  "w-full rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-sm text-slate-900 outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-100";
  ...
  return (
    <div className="space-y-3">
      ...
      <div
        ref={wrap}
        onClick={onPick}
        className="relative w-full cursor-crosshair overflow-hidden rounded-xl bg-slate-900 ring-1 ring-slate-200"
      >
        ...
        {pins.map((p) => (
          <span
            ...
            className="pointer-events-none absolute grid h-6 w-6 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-brand-600 text-[11px] font-black text-white ring-2 ring-white"
        ...
        {placing && (
          <span
            aria-hidden
            className="pointer-events-none absolute h-6 w-6 -translate-x-1/2 -translate-y-1/2 animate-pulse rounded-full bg-teal-400 ring-2 ring-white"
      ...
      {placing && (
        <div className="rounded-xl border-2 border-teal-300 bg-teal-50/50 p-3">
          <p className="mb-2 text-xs font-bold text-teal-900">
          ...
          <p className="mt-1.5 text-[11px] text-teal-800/80">
          ...
          <ul className="divide-y divide-slate-100">
            {pins.map((p) => (
              <li key={p.id} className="flex items-start gap-2.5 px-3 py-2.5">
                <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-brand-600 text-[10px] font-black text-white">
                  {p.label}
                </span>
...
function RemovePin({ id }: { id: string }) {
  ...
  return (
    <button
      ...
      className="shrink-0 text-slate-300 transition hover:text-rose-600 disabled:opacity-60"
    >
