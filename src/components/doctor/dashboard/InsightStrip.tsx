  ...
  return (
    <section className="mb-5">
      <div className="mb-2.5 flex items-center gap-2">
        <h2 className="font-display text-[15px] font-bold text-ink sm:text-base">
          Worth a look
        </h2>
      ...
      <ul className="grid grid-cols-2 gap-2.5 sm:gap-3 xl:grid-cols-4">
        {items.map((item) => {
          ...
          return (
            <li
              key={item.title}
              className="portal-enter flex items-start gap-2.5 rounded-2xl bg-white/[0.04] p-3 shadow-[0_18px_44px_-24px_rgba(0,0,0,0.75)] ring-1 ring-white/10 sm:gap-3 sm:p-4"
            >
              <span
              ...
              <div className="min-w-0 flex-1">
                {item.metric && (
                  <p className="font-display text-lg font-bold leading-none tabular-nums text-ink sm:text-xl">
                    {item.metric}
                  </p>
                )}
                <p
                  className={`text-[13px] font-bold leading-snug text-ink sm:text-sm ${
                    item.metric ? "mt-1" : ""
                  }`}
                >
                  {item.title}
                </p>
                <p className="mt-1 text-[11px] leading-relaxed text-ink-muted sm:text-xs">
                  {item.body}
                </p>
              </div>
            </li>
...
export function InsightStripSkeleton() {
  return (
    <section className="mb-7">
      ...
      <ul className="grid grid-cols-2 gap-2.5 sm:gap-3 xl:grid-cols-4">
        {[0, 1, 2, 3].map((i) => (
          <li key={i} className="h-24 animate-pulse rounded-2xl bg-white/[0.06]" />
        ))}
      </ul>
    </section>
  );
...
const INSIGHT_SKINS: Record<string, string> = {
  calendar: "bg-brand-500/15 text-brand-300",
  money: "bg-teal-500/15 text-teal-300",
  people: "bg-violet-500/15 text-violet-300",
  star: "bg-amber-500/15 text-amber-300",
  clock: "bg-rose-500/15 text-rose-300",
  spark: "bg-white/[0.06] text-ink-soft",
};
