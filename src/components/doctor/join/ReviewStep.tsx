...
export default async function ReviewStep({
  ...
  if (status === "PENDING") {
    return (
      <div className="rounded-[10px] border border-azure-200 bg-azure-50 p-8 text-center">
        <h2 className="text-lg font-bold text-azure-900">
          Your application is with our team
        </h2>
        <p className="mx-auto mt-2 max-w-md text-sm text-azure-800">
  ...
  return (
    <div className="space-y-5">
      {gaps.length > 0 ? (
        <div className="rounded-[10px] border border-gold-200 bg-gold-50 p-5">
          ...
          <ul className="mt-2 space-y-1">
            {gaps.map((g) => (
              <li key={g.key} className="text-sm text-gold-900">
                ·{" "}
        ...
        <div className="rounded-[10px] border border-mint-200 bg-mint-50 p-5">
          <p className="font-bold text-mint-900">Everything we need is here</p>
      ...
      <div className="rounded-[10px] border border-graphite-200 bg-white p-5 text-sm text-graphite-600">
        <p className="font-bold text-graphite-900">What happens next</p>
